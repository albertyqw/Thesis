...
function inputActivation = test_input(M, cd_k, c, b, x, state)      
    Nh = size(c, 1);
    Ni = size(b, 1);
    ...
    title(sprintf("State: %s, Original Image %f", state, i))

    nexttile
    imshow(reshape(vk, 28, 28)); % reshape visible layer
    title(sprintf("State: %s, Reconstructed Image %f", state, i))
end